...
const HoldingAccountSummaryTable: React.FC<HoldingAccountSummaryTableProps> = ({ accounts, holdings: holdings }) => {
  ...
  const getPortfolioJson = () => {
    ...
    const exportData = allExportRows.map(row => ({
      ...
      currentPrice: Number(row.currentPrice?.toFixed(2) || 0),
  ...
  const holdingRows = holdings.map(holding => {
    const valueUSD = convertToUSD(holding.price! * holding.quantity, holding.currency);
    return {
      id: holding.ticker,
      name: holding.ticker,
      ...
      currency: 'USD',
      icon: holding.assetType === 'stock' ? <TrendingUp /> : <Bitcoin />,
      originalValue: holding.averagePrice * holding.quantity,
  ...
  const allRows = [...holdingRows, ...accountRows];
            ...
            {summaryRows.map((item: typeof allRows[number] & { percentage: number }) => (
              <tr key={item.id}>
                ...
                <td className="px-3 py-2 whitespace-nowrap text-right text-xs sm:px-6 sm:py-4 sm:text-sm">
                  <span className="font-mono text-gray-700">{formatPrice(item.valueUSD, item.currency)} <span className="text-xs text-gray-400">USD</span></span>
                  {item.originalCurrency !== 'USD' && (
                    <div className="text-xs text-gray-400">{formatPrice(item.originalValue, item.originalCurrency)}</div>
                  )}
                </td>
                <td className="px-3 py-2 whitespace-nowrap text-right text-xs sm:px-6 sm:py-4 sm:text-sm">
